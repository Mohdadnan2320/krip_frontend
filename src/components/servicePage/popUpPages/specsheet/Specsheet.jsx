...
export default function Specsheet() {
  ...
  const [specSheetData, setspecSheetData] = useState(null);
  ...
  //   console.log(selectedTemplate.title)

  //   try {
  //     const response = await axios.post("http://localhost:8080/api/specsheet", formData);
  //     console.log(response.data.content)
  //     setResponseData(response.data)
  //     alert("hogaay");
  //   } catch (error) {
  //     console.error("Error:", error);
  //     alert("An error occurred while generating the spec sheet.");
  //   }
  // }
    ...
    const fetchSpecSheet = async () => {
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_BACKEND_URL}/api/specsheet`
          ,
          formData
        );
        ...
        setspecSheetData(parsedContent); // store response
      } catch (error) {
        console.error("Failed to fetch spec sheet", error);
      } finally {
        setLoading(false);
      }
    };
              ...
              <div className="lg:mt-5">
                <div className="border-b-2 border-gray-500 border-dashed py-4">
                  ...
                  <div className="w-full rounded-xl border-2 border-[#265b5bf1] px-4 py-3 flex justify-between mt-4">
                    ...
                    <span className="text-lg">
                      <span>{specSheetData?.totalPrice}</span>
                    </span>
                  </div>
                </div>
                ...
                <div className="border-b-2 border-gray-500 border-dashed py-4">
                  ...
                  <div className="w-full rounded-xl flex justify-between mt-2">
                    ...
                    <span className="">
                      {specSheetData?.estimatedTimeOfDelivery}
                    </span>
                  </div>
                </div>
                ...
                <div className="py-4">
                  ...
                  <div className="w-full rounded-lg border-2 border-[#265b5bf1] lg:px-4 px-2 py-3 flex flex-col justify-between mt-4">
                    {specSheetData.paymentOptions.map((option) => (
                      <label
                        key={option.id}
                ...
                <div className="flex lg:gap-10 gap-5 lg:mt-4 mt-2 justify-center">
                  <button className="text-black lg:text-sm text-[12px] cursor-pointer bg-[#2cef80] lg:px-7 px-6 lg:py-2 py-1 rounded-lg font-semibold">
                    Place Order
                  </button>
                  <button
                    onClick={() =>
...
// </div> */}
                  ...
                  <div className="mt-4 text-sm">
                    ...
                    <p>Service: {specSheetData?.serviceDetails.service}</p>
                    <p>Template: {specSheetData?.serviceDetails.template}</p>
                  </div>
          ...
          {/* <p className="text-sm text-center italic">
            Would you like to approve this Spec Sheet or request any changes?
          </p> */}
        </div>
      )}
    </div>
